...
public class BookResource {
    ...
    public Response getBooks(){
        List<Book> books = service.findAll(); //Get all the books from the database
    ...
    @APIResponse(responseCode = "404", description = "No book found with given identifier")
    ...
    @Produces(TEXT_PLAIN)
    @Path("/count")
    @Operation(summary = "Returns the number of books from the database as a plain text")
    ...
    @DELETE
    @Path("/{id}")
    @Operation(summary = "Deletes an existing book based on the provided identifier")
